...
class Functions{
    ...
    public static function SoftwareLink($type, $ip, $c_port, $q_port, $s_port)
    {
        $links = [
        ...
        if (!isset($links[$type])) { $type = str_replace("_", "", $type); }

        // INSERT DATA INTO STATIC LINK - CONVERT SPECIAL CHARACTERS - RETURN
        return htmlentities(str_replace(array("{IP}", "{C_PORT}", "{Q_PORT}", "{S_PORT}"), array($ip, $c_port, $q_port, $s_port), $links[$type]), ENT_QUOTES);
    }

    public static function GameTypeScheme($type)
    {
        $scheme = [
            "bfbc2"         => "tcp",
            "bf3"           => "tcp",
        ...
        return isset($scheme[$type]) ? $scheme[$type] : "udp";
    }

    public static function PortConvert($type, $c_port, $q_port, $s_port)
    {
        switch ($type)
        {
        ...
        return [intval($c_port), intval($q_port), intval($s_port)];
    }
    ...
    public static function ParserColor($string, $type)
    {
        switch($type)
        {
            case "1":
    ...
    public static function UnPack($string, $format)
    {
        list(,$string) = @unpack($format, $string);
        return $string;
    }

    public static function CutByte(&$buffer, $length)
    {
    ...
    public static function CutString(&$buffer, $start_byte = 0, $end_marker = "\x00")
    {
    ...
    public static function CutPascal(&$buffer, $start_byte = 1, $length_adjust = 0, $end_byte = 0)
    {
    ...
    public static function GetString($buffer, $start_byte = 0, $end_marker = "\x00")
    {
        $buffer = substr($buffer, $start_byte);
        $length = strpos($buffer, $end_marker);
    ...
    public static function GSEncrypt($type, &$buffer, $encrypt = FALSE)
    {
        $master_key = pack("H*",
